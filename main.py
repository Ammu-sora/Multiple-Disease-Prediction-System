...
with st.sidebar:
    ...
    selected = option_menu(
        ...
        ['Diabetes Prediction', 'Heart Disease Prediction', 'Parkinsons Prediction', 'Calories Burnt Prediction','Dashboard'],
        icons=['activity','heart','person','person-gear','bar-chart'],
...
def dashboard_page():
    ...
    # === Enhanced Prediction Distribution Section ===
    st.markdown("### 📊 Prediction Distribution")

    distribution_type = st.selectbox(
        "Select Distribution View",
        ["Overall", "By Gender", "By Prediction Result"]
    )

    if distribution_type == "Overall":
        pie_df = pd.DataFrame({
            "Disease": ["Diabetes", "Heart", "Parkinson's", "Calories"],
            "Count": [len(diabetes_df), len(heart_df), len(parkinsons_df), len(calories_df)]
        })
        pie_chart = px.pie(pie_df, names='Disease', values='Count', hole=0.4,
                           title="Overall Prediction Distribution")
        st.plotly_chart(pie_chart, use_container_width=True)

    elif distribution_type == "By Gender":
        # Collect gender-wise count per disease
        def get_gender_dist(df):
            return df['gender'].str.capitalize().value_counts() if 'gender' in df.columns else pd.Series()
        ...
        for name, df in zip(["Diabetes", "Heart", "Parkinson's", "Calories"],
                            [diabetes_df, heart_df, parkinsons_df, calories_df]):
            gender_counts = get_gender_dist(df)
            print(gender_counts)
            for gender, count in gender_counts.items():
                gender_df = pd.concat([gender_df, pd.DataFrame({
                    "Disease": [name],
                    "Gender": [gender],
                    "Count": [count]
                })], ignore_index=True)

        fig = px.bar(gender_df, x="Disease", y="Count", color="Gender", barmode="group",
                     title="Prediction Distribution by Gender")
        st.plotly_chart(fig, use_container_width=True)

    elif distribution_type == "By Prediction Result":
        # Build result-based chart
        result_df = pd.DataFrame(columns=["Disease", "Result", "Count"])
        ...
        fig = px.bar(result_df, x="Disease", y="Count", color="Result", barmode="group",
